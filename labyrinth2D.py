...
class Labyrinth2D(Labyrinth):
    ...
    def __init__(self, x, y, wgrd, hwallh, wwallv):
        ...
        self.ground = pygame.transform.scale(
            self.ground, (self.wgrd, self.wgrd))

        self.groundend = pygame.image.load("images/groundend.png")
        self.groundend = pygame.transform.scale(
            self.groundend, (self.wgrd, self.wgrd))

        self.wallh = pygame.image.load("images/wallh.png")
        self.wallh = pygame.transform.scale(
            self.wallh, (self.wgrd, self.hwallh))

        self.wallv = pygame.image.load("images/wallv.png")
        self.wallv = pygame.transform.scale(
            self.wallv, (self.wwallv, self.wgrd))

        self.angle = pygame.image.load("images/angle.png")
        self.angle = pygame.transform.scale(
            self.angle, (self.wwallv, self.hwallh))
    ...
    def draw_lab(self):
        ...
        self.window.blit(self.wallh, (xmax, y_col -
                                      self.wgrd - self.hwallh / 2))
        ...
        for row in self.laby:
            for col in row:
                # Borders of the laby
                if y == 0:
                    wall = pygame.Rect(x, y - self.hwallh / 2,
                                       self.wgrd, self.hwallh)
                    self.window.blit(self.wallh, (x, y - self.hwallh / 2))
                    self.mywalls.append(wall)
                if y == (self.width - 1) * self.wgrd:
                    wall = pygame.Rect(x, y + self.wgrd -
                                       self.hwallh / 2, self.wgrd, self.hwallh)
                    self.window.blit(
                        self.wallh, (x, y + self.wgrd - self.hwallh / 2))
                    self.mywalls.append(wall)
                if x == 0 and y != 0:
                    wall = pygame.Rect(x - self.wwallv / 2,
                                       y, self.wwallv, self.wgrd)
                    self.window.blit(self.wallv, (x - self.wwallv / 2, y))
                    self.mywalls.append(wall)
                if x == (self.height - 1) * self.wgrd and y != (self.width - 1) * self.wgrd:
                    wall = pygame.Rect(
                        x + self.wgrd - self.wwallv / 2, y, self.wwallv, self.wgrd)
                    self.window.blit(
                        self.wallv, (x + self.wgrd - self.wwallv / 2, y))
                    self.mywalls.append(wall)
                ...
                if self.laby[xl][yl] == ' |':
                    wall = pygame.Rect(
                        yl * self.wgrd + self.wgrd - self.wwallv / 2, xl * self.wgrd, self.wwallv, self.wgrd)
